...
from audio.processor import SUPPORTED_EXTENSIONS, check_ffmpeg, get_audio_info, process_uploaded_file
...
from exports.exporter import export_as_txt
...
st.set_page_config(page_title="Transcribe - MLabs", page_icon="🎙️", layout="wide")
init_ui()
require_login()
sidebar_navigation(current="transcribe")
...
render_page_header(
    "New Transcription",
    f"{team_name} upload flow with mobile and desktop-specific layouts and simplified export options.",
)
...
if mobile:
    config_section = st.container()
    upload_section = st.container()
else:
    config_section, upload_section = st.columns([1, 1.45], gap="large")
...
with config_section:
    with st.container(border=True):
        ...
        summary_requested = st.checkbox(
            "Generate transcript summary (OpenAI)",
            value=False,
        ...
        st.info("Long audio is split into 10-minute chunks and stitched after transcription.")
...
with upload_section:
    with st.container(border=True):
        ...
        if not uploaded_file:
            st.info("Add an audio file to preview it here, review metadata, and start transcription.")
        else:
            ...
            if mobile:
                ...
                with m2:
                    st.metric("Size", f"{info.get('size_mb', 0)} MB")
                ...
                try:
                    ...
                    with result_area:
                        ...
                        if summary_requested:
                            st.markdown("### Summary")
                            if summary_result:
                                st.caption(f"Generated with {summary_result['model_used']}.")
                                st.text_area("Summary", value=summary_result["summary"], height=220)
                            else:
                                st.warning(f"Summary could not be generated: {summary_error}")

                        include_summary = st.checkbox(
                            "Include summary in TXT export",
                            value=bool(summary_result),
                            disabled=not bool(summary_result),
                            key=f"tx_export_summary_{tid}",
                        )

                        btn_cols = st.columns(2 if mobile else 3)
                        with btn_cols[0]:
                            st.download_button(
                                "Download TXT",
                                export_as_txt(
                        ...
                        if not mobile:
                            with btn_cols[2]:
                                st.caption("JSON and CSV exports were removed from this view.")
